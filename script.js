  ...
  data.forEach(opportunity => {
    geocoder.geocode({ address: opportunity.location }, (results, status) => {
      if (status === "OK") {
        ...
        marker.addListener("click", () => {infoWindow.open(map, marker); document.getElementById("data").innerHTML=opportunity.opportunityname+opportunity.description+opportunity.link});
        markers.push(marker);
      }
    });
  });
